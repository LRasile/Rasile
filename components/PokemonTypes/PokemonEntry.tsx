...
export default function PokemonEntry({ pokemon, typeEfficacy, build, eager }: PokemonEntryProps) {
  ...
  return (
    <div
      ...
      style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'flex-start', position: 'relative' }}
    >
      ...
      >
        ...
        <a
          ...
          href={`https://bulbapedia.bulbagarden.net/wiki/${GetUrlName(pokemon)}_(Pokémon)`}
          style={{ fontSize: 17, fontWeight: 'bold', display: 'block' }}
        >
          ...
          {pokemon.abilities.map((a) => (
            <a
              ...
              style={{
                margin: '0.5em',
                display: 'inline',
                fontStyle: a.isHidden ? 'italic' : 'normal',
                opacity: a.isHidden ? 0.55 : 0.85,
                color: 'inherit',
                textDecoration: 'none',
              }}
            >
        ...
        {showBuild && build && (
          ...
          >
            ...
            >
              ...
              <div
                style={{ fontWeight: 'bold', fontSize: '1.1rem', textTransform: 'capitalize', marginBottom: '1rem' }}
              >
                {formatPokemonName(pokemon.name)} — Build
              </div>
              {build.ability && (
            ...
            return (
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                ...
                >
                  <div
                    style={{
                      width: `${Math.min(value / 200, 1) * 100}%`,
                      background: color,
                      height: '100%',
                      borderRadius: 3,
                    }}
      ...
      <div style={{ flex: '1 1 280px', minWidth: 280, display: 'flex', gap: 4 }}>
        {[
          { label: '4', factor: 4, bg: 'rgba(252,129,129,0.15)' },
          { label: '2', factor: 2, bg: 'rgba(252,129,129,0.15)' },
          { label: '0.5', factor: 0.5, bg: 'rgba(104,211,145,0.12)' },
          { label: '0.25', factor: 0.25, bg: 'rgba(104,211,145,0.12)' },
          { label: '0', factor: 0, bg: 'rgba(104,211,145,0.12)' },
        ].map(({ label, factor, bg }) => {
          ...
          return (
            ...
            >
              ...
              {types.map((t) => (
                <div key={pokemon.id + ':' + t.name} className={`pokemonType pokemonType${t.name}`} style={{}}>
                  {t.name}
                </div>
              ))}
            </div>
